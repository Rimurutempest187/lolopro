...
async def basket(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Wait for dice animation short time (value available in dice object)
...
async def givecoin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # two forms:
    # 1) reply to user: /givecoin <amount>
    # 2) /givecoin <user_id> <amount>
    target_user_id = None
    amount = None
    ...
    if last_daily:
        try:
            ...
            if datetime.now().date() == last_dt.date():
                # calculate remaining until next midnight local
...
async def shop_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "buy":
        ...
        if user["coins"] < price:
            await query.answer(f"❌ Coins မလောက်ပါဘူး! လိုအပ်တာ: {price:,} coins", show_alert=True)
            return
...
async def message_counter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data["group_messages"][chat_id] >= data.get("drop_count", 10):
        data["group_messages"][chat_id] = 0
        if data.get("cards"):
...
async def upload(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Expecting: reply with photo + caption "Name | Movie | Rarity"
...
async def gift(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Unified gift handler: /gift coin <amount> [user_id] OR /gift card <amount> [user_id]"""
    if not update.message:
        return
    caller = update.effective_user.id
    if not is_admin(caller):
...
async def backup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        with open(DATA_FILE, "rb") as f:
            await update.message.reply_document(document=f, filename=f"backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", caption="💾 <b>Data Backup</b>\n\nBackup ပြီးပါပြီ!", parse_mode=ParseMode.HTML)
    except Exception:
...
async def allclear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("⚠️ <b>သတိပြုပါ!</b>\n\nData အားလုံး ဖျက်မှာသေချာပါသလား?\nဒီလုပ်ငန်းကို ပြန်ပြင်လို့မရပါဘူး!", reply_markup=reply_markup, parse_mode=ParseMode.HTML)
